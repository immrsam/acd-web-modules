...
// Helper function to prevent form submission on Enter key
function preventSubmitOnEnter(inputElement) {
    inputElement.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            // Move focus to next enabled input
...
function setupScanPage() {
    ...
    function showNewOrderModal(sop, rating) {
    ...
    // Function to hide the new order modal
    ...
    // Existing order form submission - modified to show modal when order not found
    if (orderForm) {
        orderForm.addEventListener('submit', function(e) {
            ...
            if (jsonData.orders[orderKey]) {
                // Existing order processing code remains the same
                const now = new Date();
                const timestamp = now.getFullYear().toString() + 
                    String(now.getMonth() + 1).padStart(2, '0') + 
                    String(now.getDate()).padStart(2, '0') + 
            ...
            // If order not found, show the modal to create it
            showNewOrderModal(sop, rating);
        });
    }

    // Rest of the existing setupScanPage function remains the same
    if (exportBtn) {
